...
def tache_serveur():
  while True:
    ...
    thread_client = threading.Thread(target=tache_client, args=(clientsocket,address))
    thread_client.start()
...
settings = dict(
  ...
  autoescape = None,
  cookie_secret = str(os.urandom(45)),xsrf_cookies=True
)
...
try:
  ...
  application = tornado.web.Application([(r'/',wwwHandler), (r'/ws',WebSocketCommun) ],autoreload=True, **settings)
