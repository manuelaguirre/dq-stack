...
class ClientRenderer(Renderer):
    ...
    def _select_themes(self):
        ...
        for index in range(num_rows):
            rows.append(2/9*self.SCREEN_HEIGHT + 7/9*self.SCREEN_HEIGHT/(2*num_rows+1)*(2*index+1))
        for index in range(len(self.themes)):
            self.display_button(self.themes[index], columns[index % 2], rows[math.floor(index/2)])
